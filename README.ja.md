...
| `measure-log` | 広告施策の実行記録をタイムスタンプ付きメモで管理 |
...
# 施策にメモを追記
aads measure-log --id <entry-id> --note "ACOS 15% → 10%に改善"
...
広告施策（キャンペーン変更、最適化）をタイムスタンプ付きメモで記録・管理します。データは作業ディレクトリの `./data/measure-log.json` に保存されます。
...
| `--add` | 新しいエントリを追加 |
| `--remove <id>` | エントリを削除 |
| `--id <entry-id>` | 対象エントリID（`--note` と併用） |
| `--note <text>` | メモを追記（`--id` と併用で既存エントリに追記、`--add` と併用で初回メモ付き作成） |
| `--pattern <id>` | 施策パターンID |
| `--name <text>` | 施策名 |
